...
<table border = "1" style="font-szie:20px" width="50%">
	...
	<?php
	$bignm = 0;
	$bignum = (int)($va['allga']['num']) + 45;
	
	$bignum = $bignum <100 ? '00'.$bignum : ($bignum >100 && $bignum < 1000 ? '0'.$bignum : $bignum);

	$sql1111 = "SELECT * FROM `yingin10000` left join yingin192 on yingin192.yingin192_id = yingin10000.yingin192_id  WHERE yingin10000_num ='$bignum'";
	$result1111 = mysqli_query($database_link,$sql1111) or die(mysqli_error());
	$row1111 = mysqli_fetch_assoc($result1111);
	?>
		<td colspan="2">大總格</td>
		<td><?php echo $row1111['yingin10000_num']; ?></td>
		<td><?php echo $row1111['goodorbad']; ?></td>
		<td><?php echo $row1111['mean']; ?></td>
	</tr>
