...
class crawler extends SqlTool{
	...
	private function arrjson($Jsonarr){
		...
		for($i=0;$i<count($Jsonarr['XML_Head']['Infos']['Info']);$i++){
	    	    ...
	    	    $info[$i]['px']=$Jsonarr['XML_Head']['Infos']['Info'][$i]['Px'];
	    	    $info[$i]['py']=$Jsonarr['XML_Head']['Infos']['Info'][$i]['Py'];
	    	    $info[$i]['tel']=$Jsonarr['XML_Head']['Infos']['Info'][$i]['Tel'];
	    	    $info[$i]['ticketinfo']=$Jsonarr['XML_Head']['Infos']['Info'][$i]['Ticketinfo'];
	    	    $info[$i]['toldescribe']=$Jsonarr['XML_Head']['Infos']['Info'][$i]['Toldescribe'];
	    	    $info[$i]['travellinginfo']=$Jsonarr['XML_Head']['Infos']['Info'][$i]['Travellinginfo'];
	    	    ...
	    	    $this->CUspotdata($city[$i],$area[$i],$address[$i],$class1[$i],$class2[$i],$class3[$i],$spotname[$i],$opentime[$i],$de_jsoninfo[$i],$checkinfoupdate[$i],$description[$i]['description']);
	    }
	}
	private function CUspotdata($city,$area,$address,$class1,$class2,$class3,$spotname,$opentime,$info,$checkinfoupdate,$description){
		$search['spotname']=$spotname;
		$search['checkupdate']=$checkinfoupdate;
	
			    $sql="UPDATE `spot` SET `checkupdate`='$checkinfoupdate' ,`info`='$info' where  spotname='$spotname'";
