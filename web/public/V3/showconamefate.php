<?php
    if (!empty($_POST)) {
        require '../V3/class/chineseCoName.php';
        $chinese_co_name = new chineseCoName;
        $name_fate = json_decode($chinese_co_name->fire($_POST['name'], $_POST['allname']), true);
    } else {
                    ...
                    <h1>
                        <?php
                            foreach ($name_fate['disintegration_name'] as $key => $val) {
                                ...
                                echo '<d style="font-family:標楷體;font-size:24px;width:48px;line-height:55px;border:1px dashed #ccc;padding:8px;margin-bottom: 5px;">' . $val['name'] . '</d>';
...
<div class="work section second" id="skills">
	<div class="container">
		...
		<ul class="work-list">
            ...
            <li>大總格和數</li>
			<li data-tooltip="<?php echo $name_fate['big_total_num']['article']['point'] . '  ';?>"><a href="#"><?php echo $name_fate['big_total_num']['article']['goodorbad'] . '  ';?></a><?php echo $name_fate['big_total_num']['article']['mean'] . '  -  ' .$name_fate['big_total_num']['article']['yingin10000_num'];?></php></li>
